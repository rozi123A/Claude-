...
export default function SpinWheelSection({ user, onReward }: SpinWheelSectionProps) {
  ...
  async function handleClaimSpinAd() {
    ...
    toast({ title: "🎉 مبروك!", description: `حصلت على دورة إضافية!` });
  ...
  async function handleSpin() {
    ...
    try {
      ...
      if (!data.success) { toast({ title:"تنبيه", description: data.message||"فشل", variant:"destructive" }); setIsSpinning(false); return; }
      ...
      const dur = 4000; const t0 = Date.now(); const r0 = rotation;
      ...
      const animate = () => {
        ...
        if (p < 1) { requestAnimationFrame(animate); }
          ...
          toast({ title:"🎉 مبروك!", description:`ربحت ${data.prize} نقطة!` });
  ...
  return (
    <>
      {showAd && (
        <AdOverlay seconds={15} rewardLabel="🎡 دورة إضافية"
          onClaim={handleClaimSpinAd} onClose={() => setShowAd(false)} />
      )}
      ...
      <Card className="bg-gradient-to-b from-slate-900/80 to-slate-950 border-slate-700/50 shadow-xl overflow-hidden">
        ...
        <CardContent className="pt-8 pb-6 space-y-6">
          ...
          {/* Ad-spin section — shows ONLY when free spins = 0 */}
          {noFreeSpins && (
            <div className="space-y-3">
              <div className="bg-slate-800/60 border border-purple-700/40 rounded-xl p-4 text-center">
                <p className="text-yellow-400 font-bold text-sm">انتهت دوراتك المجانية اليوم!</p>
                <p className="text-gray-400 text-xs mt-1">شاهد إعلاناً للحصول على دورة إضافية</p>
              </div>

              {/* Ad-spin progress */}
              <div className="bg-slate-900/40 p-3 rounded-xl border border-slate-800/50">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs text-gray-400 font-bold">إعلانات السبينات</span>
                  <span className="text-xs font-black" style={{ color: adSpinsLeft > 0 ? "#c4b5fd" : "#6b7280" }}>
                    {adSpinsUsed} / {MAX_AD_SPINS} مستخدمة
                  </span>
                </div>
              ...
              {adSpinsLeft > 0 ? (
                <>
                  <Button onClick={() => setShowAd(true)} disabled={isSpinning}
                    ...
                    شاهد إعلان واربح دورة 🎡
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-black" style={{ background:"rgba(255,255,255,0.2)" }}>
                      {adSpinsLeft} متبقية
                    </span>
                  </Button>
                  <p className="text-[10px] text-gray-500 text-center">كل إعلان = دورة واحدة • حتى {MAX_AD_SPINS} إعلانات يومياً</p>
                </>
              ) : (
                <div className="text-center py-3 px-4 bg-slate-800/40 rounded-xl border border-slate-700/30">
                  <p className="text-xs text-gray-500 font-bold">🌙 استُنفدت جميع إعلانات اليوم</p>
                  <p className="text-[10px] text-gray-600 mt-1">تجدد الدورات والإعلانات المجانية غداً</p>
                </div>
              )}
            </div>
          )}
