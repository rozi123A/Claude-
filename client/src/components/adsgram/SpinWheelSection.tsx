...
export default function SpinWheelSection({ user, lang, onReward, onSwitchToAds }: SpinWheelSectionProps) {
    ...
    try {
      ...
      if (result.done) {
        ...
        if (claimData.success) {
          toast({
            title: t.congrats,
            description: `${t.extra_spin_reward}: ${claimData.reward}`,
          });
          onReward(
    ...
    try {
      ...
      const animate = () => {
        ...
        if (progress < 1) {
          ...
          toast({
            title: t.congrats,
            description: `${t.won_points.replace("points", String(data.prize))} ${data.prize} PTS`,
          });
          onReward(
