  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      if (result.success && result.reward) {
        ...
        // Server says already claimed — sync the timer
        if (result.nextClaim) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      >
        ...
        <div style={{
          ...
          <div style={{
            ...
            borderRadius: 10,
            boxShadow: canClaim
              ? "0 10px 30px rgba(124,58,237,0.55),inset 0 1px 0 rgba(255,255,255,0.15)"
          ...
          <div style={{
            ...
            <div style={{
              width: "100%", height: 9,
              background: canClaim ? "linear-gradient(90deg,#fbbf24,#f59e0b)" : "linear-gradient(90deg,#6b7280,#4b5563)",
              borderRadius: 3,
            }} />
            {/* Bow (only when available) */}
            {canClaim && (
              <div style={{ position: "absolute", top: -14, display: "flex", gap: 1 }}>
                <div style={{
                  ...
                  boxShadow: "0 2px 5px rgba(251,191,36,0.5)",
                }} />
                <div style={{
                  width: 16, height: 12,
                  background: "linear-gradient(135deg,#fde68a,#fbbf24)",
        ...
        {showReward && (
          <div style={{
            ...
            color: "#fbbf24", textShadow: "0 0 18px rgba(251,191,36,0.9)",
            animation: "giftRewardPop 0.5s ease-out forwards",
      ...
      <div className="text-center">
        {canClaim ? (
          <>
            <p className="text-xs font-black text-yellow-400" style={{ animation: "giftPulse 1.5s ease-in-out infinite" }}>{t.daily_gift_ready}</p>
          </>
        ) : (
          <>
      ...
      <style>{`
        ...
        @keyframes giftRewardPop {
          0%{opacity:0;transform:translate(-50%,-50%)scale(.4)}
          55%{opacity:1;transform:translate(-50%,-90%)scale(1.25)}
