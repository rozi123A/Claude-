...
export default function WatchAdsSection({ user, lang, onReward }: WatchAdsSectionProps) {
  ...
  const handleWatchAd = async () => {
    ...
    setLoading(true);
    try {
      // 1. Get one-time server token before showing ad
      ...
      // 2. Wait for Adsgram SDK to be available
      const Adsgram = await waitForAdsgram(5000);

      // 3. Always create a fresh controller — never reuse stale instances
      const controller = Adsgram.init({ blockId });

      setLoading(false);

      // 4. Show ad — resolves only when user finishes watching
      await controller.show();

      // 5. Claim reward after confirmed full ad view
      ...
      } else if (error?.type === "banner_not_found" || error?.message?.toLowerCase().includes("not found")) {
        toast({ title: "خطأ في الإعلان", description: "معرّف الإعلان غير صالح. يرجى التواصل مع المطور.", variant: "destructive" });
        console.error("[Adsgram] blockId not found:", blockId, error);
      } else {
