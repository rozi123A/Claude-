import { useState, useEffect } from "react";
    ...
    export default function WatchAdsSection({ user, lang, onReward }: WatchAdsSectionProps) {
      ...
      const handleWatchAd = async () => {
        ...
        try {
          ...
          if (claimData.success) {
            ...
            toast({ title: t.congrats || "🎉 أحسنت!", description: `${t.earned_points || "ربحت"} +${claimData.reward} ${t.points}` });
          ...
          if (error?.type === "show") {
            toast({ title: t.notice || "تنبيه", description: "يجب مشاهدة الإعلان كاملاً للحصول على النقاط", variant: "destructive" });
          } else {
      ...
      const canWatch = cooldownRemaining === 0 && user.todayAds < 10 && !loading;
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          >
            ...
            {loading ? t.loading : cooldownRemaining > 0 ? `${t.wait} ${Math.ceil(cooldownRemaining)}${t.seconds}` : t.watch_ad}
          </button>
