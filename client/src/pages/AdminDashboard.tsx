import { useState, useEffect, useCallback } from "react";
  ...
  import {
    ...
    Eye, EyeOff, RefreshCw, Ban, CheckCircle, XCircle,
    Megaphone, LogOut, Clock, Star
  } from "lucide-react";
  ...
  export default function AdminDashboard() {
      ...
      <div style={{ minHeight: "100vh", background: "#070711", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        ...
        <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 400 }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <h1 style={{ fontSize: 26, fontWeight: 900, background: "linear-gradient(135deg,#A78BFA,#60A5FA)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", margin: 0 }}>لوحة التحكم</h1>
          ...
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 24, padding: 28 }}>
            ...
            <button
              ...
              style={{ width: "100%", height: 52, borderRadius: 18, border: "none", background: secret.trim() ? "linear-gradient(135deg,#7C3AED,#4F46E5)" : "rgba(255,255,255,0.05)", color: secret.trim() ? "#fff" : "rgba(255,255,255,0.2)", fontWeight: 900, fontSize: 15, cursor: secret.trim() ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, transition: "all 0.3s", boxShadow: secret.trim() ? "0 6px 24px rgba(124,58,237,0.35)" : "none" }}
            >
    ...
    // ======================== DASHBOARD ========================
    const TABS = [
      { id: "stats", icon: <BarChart3 size={16} />, label: "الإحصائيات" },
      { id: "users", icon: <Users size={16} />, label: "المستخدمون" },
      { id: "withdrawals", icon: <Wallet size={16} />, label: "السحوبات" },
      { id: "broadcast", icon: <Megaphone size={16} />, label: "البث" },
    ] as const;
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 0 24px" }}>
            <div>
              ...
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", fontWeight: 600, margin: 0 }}>Start Coin Bot — Admin Panel</p>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => statsQ.refetch()} style={{ width: 40, height: 40, borderRadius: 12, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.5)" }}>
                <RefreshCw size={16} />
              </button>
              <button onClick={handleLogout} style={{ height: 40, borderRadius: 12, border: "1px solid rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.08)", cursor: "pointer", padding: "0 16px", display: "flex", alignItems: "center", gap: 8, color: "#FCA5A5", fontWeight: 700, fontSize: 12 }}>
                <LogOut size={15} /> خروج
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 24, padding: "6px", background: "rgba(255,255,255,0.03)", borderRadius: 18, border: "1px solid rgba(255,255,255,0.06)" }}>
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, height: 40, borderRadius: 13, border: "none", background: tab === t.id ? "rgba(139,92,246,0.25)" : "transparent", color: tab === t.id ? "#A78BFA" : "rgba(255,255,255,0.35)", fontWeight: 800, fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, transition: "all 0.2s", boxShadow: tab === t.id ? "0 0 0 1px rgba(139,92,246,0.3)" : "none" }}>
                {t.icon} {t.label}
              </button>
            ))}
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {statsQ.isLoading ? (
                <div style={{ display: "flex", justifyContent: "center", padding: 48 }}>
                  <div style={{ width: 36, height: 36, border: "3px solid rgba(139,92,246,0.3)", borderTopColor: "#8B5CF6", borderRadius: "50%", animation: "spin 0.9s linear infinite" }} />
                </div>
              ) : (
                <>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 12 }}>
                    ...
                    <StatCard emoji="⏳" label="طلبات السحب" value={stats?.pendingWithdrawals ?? 0} color="#F59E0B" sub={stats?.pendingStars ? `⭐${fmtN(stats.pendingStars)}` : undefined} />
                  ...
                  <Card>
                    ...
                    <div style={{ padding: 16 }}>
                      {[
                        ...
                        { label: "مستخدمون نشطون (كل من دخل)", value: fmtN(stats?.totalUsers ?? 0), color: "#10B981" },
                      ].map((row, i, arr) => (
                ...
                <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
                  {usersQ.isLoading ? (
                    <div style={{ display: "flex", justifyContent: "center", padding: 32 }}>
                      <div style={{ width: 28, height: 28, border: "3px solid rgba(96,165,250,0.3)", borderTopColor: "#60A5FA", borderRadius: "50%", animation: "spin 0.9s linear infinite" }} />
                    </div>
                    ...
                    <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: u.isBanned === "true" ? "rgba(239,68,68,0.05)" : "rgba(255,255,255,0.025)", border: `1px solid ${u.isBanned === "true" ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.05)"}`, borderRadius: 14 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", margin: 0 }}>
                          {u.username ? `@${u.username} · ` : ""}{u.telegramId} · 💰 {fmtN(u.balance)} نقطة
                        </p>
                      </div>
                      <button
                        onClick={() => handleBan(u.telegramId, u.isBanned !== "true")}
                        style={{ height: 32, borderRadius: 10, border: "none", background: u.isBanned === "true" ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.12)", color: u.isBanned === "true" ? "#34D399" : "#FCA5A5", fontWeight: 700, fontSize: 11, cursor: "pointer", padding: "0 12px", display: "flex", alignItems: "center", gap: 6 }}
                      >
          ...
          {tab === "withdrawals" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {[["pending","⏳ معلقة"],["approved","✅ موافق عليها"],["rejected","❌ مرفوضة"]].map(([v, l]) => (
                  <button key={v} onClick={() => setWithdrawFilter(v)} style={{ flex: 1, height: 38, borderRadius: 12, border: `1px solid ${withdrawFilter === v ? "rgba(139,92,246,0.5)" : "rgba(255,255,255,0.08)"}`, background: withdrawFilter === v ? "rgba(139,92,246,0.2)" : "rgba(255,255,255,0.03)", color: withdrawFilter === v ? "#A78BFA" : "rgba(255,255,255,0.4)", fontWeight: 800, fontSize: 11, cursor: "pointer" }}>
                    {l}
                  </button>
                ))}
              </div>
              <Card>
                ...
                <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
                  {withdrawQ.isLoading ? (
                    <div style={{ display: "flex", justifyContent: "center", padding: 32 }}>
                      <div style={{ width: 28, height: 28, border: "3px solid rgba(245,158,11,0.3)", borderTopColor: "#F59E0B", borderRadius: "50%", animation: "spin 0.9s linear infinite" }} />
                    </div>
                  ) : withdrawQ.data?.withdrawals?.length === 0 ? (
                    ...
                    <div key={w.id} style={{ padding: "14px", background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                        ...
                        <div style={{ textAlign: "right" }}>
                          <p style={{ fontSize: 16, fontWeight: 900, color: "#FFD700" }}>⭐ {fmtN(w.stars)}</p>
                          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.35)" }}>{fmtN(w.amount)} نقطة</p>
                        </div>
                      </div>
                ...
                <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      {([["all","🌍","جميع المستخدمين","كل المستخدمين النشطين"],["inactive","😴","الغائبون (+3 أيام)","من لم يدخل منذ 3 أيام"]] as const).map(([v,e,t,d]) => (
                        <button key={v} onClick={() => setBroadcastTarget(v)} style={{ padding: "12px", borderRadius: 16, border: `1px solid ${broadcastTarget === v ? "rgba(236,72,153,0.4)" : "rgba(255,255,255,0.07)"}`, background: broadcastTarget === v ? "rgba(236,72,153,0.12)" : "rgba(255,255,255,0.02)", cursor: "pointer", textAlign: "right" }}>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={5}
                      placeholder="اكتب رسالتك هنا...&#10;&#10;مثال: 🎁 هدية مجانية! ادخل الآن واستلم 50 نقطة إضافية 🎮"
